...
$page_title  = get_the_title() ? get_the_title() : __( 'Blog', 'industrial-welding' );
		...
		<div class="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_320px] gap-8 items-start">
			<div class="max-w-4xl">
				...
				<p class="mt-5 max-w-3xl text-lg md:text-xl text-slate-300 leading-relaxed">
					<?php esc_html_e( 'Use this page for product updates, welding tips, buying guidance, and practical content that supports machine selection and procurement decisions.', 'industrial-welding' ); ?>
				</p>
		...
		<?php if ( $posts_query->have_posts() ) : ?>
			<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
				<?php while ( $posts_query->have_posts() ) : ?>
					...
					$raw_excerpt = trim( wp_strip_all_tags( (string) get_post_field( 'post_excerpt', get_the_ID() ) ) );

					if ( '' === $raw_excerpt ) {
						$raw_excerpt = wp_trim_words(
							wp_strip_all_tags( strip_shortcodes( (string) get_post_field( 'post_content', get_the_ID() ) ) ),
							24
						);
					}
